$(function () {
    ...
    function getChar(obj) {
        const word_char = new mojs.Shape({
            ...WORD_CHAR_OPTS,
            y: {[Y_SHIFT]: -125 + Y_SHIFT},
            angle: {0: 180, easing: 'cubic.in'},
            x: obj.offset + X_SHIFT,
            scaleY: {1: 1, curve: nBounceCurve},
            scaleX: {1: 1, curve: bounceCurve},
            easing: 'quad.out',
            delay: obj.delay,
        ...
        word_char.el.appendChild(character);

        character.innerHTML = obj.char;
        return word_char;
    }

    const word_char2 = getChar({offset: -150, delay: 75, char: 'w'});
    const word_char1 = getChar({offset: -205, delay: 0, char: 'm'});
    const word_char3 = getChar({offset: -240, delay: 250, char: 'x'});
    const word_char4 = getChar({offset: -110, delay: 325, char: '\''});
    const word_char5 = getChar({offset: -80, delay: 400, char: 's'});

    const word_char6 = getChar({offset: 50, delay: 675, char: 'r'});
    const word_char7 = getChar({offset: 85, delay: 600, char: 'e'});
    const word_char8 = getChar({offset: 120, delay: 850, char: 's'});
    const word_char9 = getChar({offset: 155, delay: 800, char: 'u'});
    const word_char10 = getChar({offset: 195, delay: 1000, char: 'm'});
    const word_char11 = getChar({offset: 250, delay: 940, char: 'e'});
    ...
    wordTimeline
        .add(
            ...
            word_char7, word_char8,
            word_char9, word_char10, word_char11,
            word_burst1,
            word_burst2,
            speechBurst,
            line1, line2,
    ...
    const timeline = new mojs.Timeline({
        onComplete (isForward, isYoyo) {
            ...
            const starLeft = getStart({top:"68%",left:'26%'});
            const starRight = getStart({top:"68%",left:'78%'});

            starLeft.replay();
            starRight.replay();


            function getBurst(orid) {
                const clickBurst1 = new mojs.Burst({
                    ...orid,
                    count:          8,
                    radius:         { 50: 150 },
                    children: {
            ...
            let top,left,delay;
            for (let i=0;i<6;i++){
                top = getRandom(20,window.innerHeight-20);
                left = getRandom(20,window.innerWidth-20);
                delay = getRandom(0,1200);
                getBurst({parent:initAni,top:top,left:left,delay:delay})
            }
            setTimeout(function () {
                initAni.hide("slow");
                $("#mainContent").show()
